...
//        engine.initialise();
    ...
    private long periodFPS = 40000000; //nsToS/requiredFPS
    ...
    public final void run() {
        ...
        while(running){
            ...
            timediff = end - start;
            //attempt to keep to a specified fps
            ...
            if(sleepTime>0){
                ...
                if(++noDelays >= EngineStatics.FPS.FRAMES_WITHOUT_SLEEP_OR_YEILD){
                    Thread.yield(); // allow for JVM to do other work
                    noDelays = 0;
                }
            }
            this.frameSkips = 0;
    ...
    //<editor-fold desc="FPS code">
    public boolean isDisplayFPS(){
        return this.showFPS;
    }
    private void addFPS(float FPS){
        /* No point using:
         * double[] newStore = Arrays.copyOfRange(storeFPS, 1, storeFPS.length);
         * newStore[newStore.length-1] = FPS;
         * as we want the average anyway
         */
    ...
    public static class EngineStatics{
        public static class FPS{
            ...
            /**
             * Number of frames without sleep before giving up the JVM
             */
            public static int FRAMES_WITHOUT_SLEEP_OR_YEILD = 16;
            /**
             * Number of updates that can be called before a render
             */
            public static int MAX_FRAME_SKIPS = 5;
            /**
             * Number of update times and render that can be stored
             * To get the average
             */
